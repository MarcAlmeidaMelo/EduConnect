...
export default function CommunicationPage() {
  ...
  const processMessage = (template: string, studentName: string, date?: Date) => {
    ...
    if (date && template.includes('[DATA]')) {
      processed = processed.replace(/\[DATA\]/g, format(date, "dd/MM/yyyy", {
        locale: ptBR
      }));
    }
    return processed;
  ...
  return <MainLayout>
      <div className="p-6 space-y-6">
        ...
        <div className="max-w-2xl mx-auto">
          <Card className="shadow-card">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select value={selectedStudent} onValueChange={setSelectedStudent} disabled={!selectedSerie}>
                  <SelectTrigger>
                    <SelectValue placeholder="Selecione um aluno" />
                  </SelectTrigger>
                  <SelectContent>
                    {filteredStudents.map(student => <SelectItem key={student.id_alunos} value={student.id_alunos}>
              ...
              {needsDate && <div className="space-y-2">
              ...
              {/* Message Preview */}
              {selectedMessageTemplate && selectedStudent && <div className="space-y-2">
                  <Label>Prévia da Mensagem</Label>
                  <div className="p-4 bg-muted rounded-lg border">
                    <p className="text-sm text-foreground">
                      {processMessage(selectedMessageTemplate.template, students.find(s => s.id_alunos === selectedStudent)?.nome_do_aluno || "", selectedDate)}
                    </p>
                  </div>
                </div>}

              {/* Send Button */}
              <Button onClick={handleSendMessage} disabled={isLoading} className="w-full h-12 text-base font-medium gap-2">
          ...
          {/* Warning Message */}
          {!selectedSerie && <div className="mt-4 p-4 bg-orange-50 border border-orange-200 rounded-lg flex items-start gap-3">
